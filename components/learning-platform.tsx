...
import { allProblems, allCategories } from "../lib/problems"
...
export default function LearningPlatform({ onBack }) {
  ...
  const getNewProblem = (currentElo) => {
    ...
    const availableProblems = allProblems.filter((p) => selectedCategories.includes(p.category))
    ...
    if (availableProblems.length === 0) {
      setCurrentProblem({
        ...
        topic: "System",
        difficulty: 0,
        problem: "No problems available for the selected categories. Please select a category to begin.",
        answer: "",
      })
      return
    }
    ...
    if (eligibleProblems.length === 0) {
      setCurrentProblem({
        ...
        topic: "System",
        difficulty: 0,
        problem: "You have seen all available problems for this filter. Please select more categories or reset.",
        answer: "",
      })
      return
    }
  ...
  useEffect(() => {
    ...
    if (savedElo) {
      initialElo = Number.parseInt(savedElo, 10)
  ...
  const handleSubmit = (e) => {
    ...
    if (correct) {
      ...
      setFeedback({
        type: "correct",
        message: `Correct! ELO ${eloChange >= 0 ? "+" : ""}${eloChange}`,
      ...
      setFeedback({
        type: "incorrect",
        message: `Incorrect. ELO ${eloChange}`,
        correctAnswer: currentProblem.answer,
      })
    }
  }
              ...
              <div className="relative">
                ...
                <AnimatePresence>
                  {isCategorySelectorOpen && (
                    ...
                    >
                      <h3 className="text-sm font-semibold text-foreground mb-3">Filter Problem Categories</h3>
                      <div className="space-y-4">
                        {/* Mathematics Section */}
                        <div>
                          <h4 className="text-xs font-medium text-primary mb-2 uppercase tracking-wide">Mathematics</h4>
                          <div className="max-h-32 overflow-y-auto space-y-1 pr-2 custom-scrollbar">
                            {["Calculus I", "Calculus II", "Linear Algebra", "Complex Analysis"].map((category) => (
                              <label
                                key={category}
                              ...
                              >
                                <div className="relative flex items-center justify-center w-5 h-5">
                                  <input
                                    ...
                                    className="appearance-none w-5 h-5 rounded-md bg-input border-2 border-border checked:bg-primary checked:border-primary transition-all cursor-pointer focus:outline-none focus:ring-2 focus:ring-ring"
                                  />
                                  <AnimatePresence>
                                    {selectedCategories.includes(category) && (
                                      ...
                                      >
                                        <Check
                                          size={14}
                                          className="text-primary-foreground absolute top-1/2 left-1/2 -translate-x-1/2 -translate-y-1/2 pointer-events-none"
                                        />
                                      </motion.div>
                                    )}
                                  </AnimatePresence>
                        ...
                        {/* Physics Section */}
                        <div>
                          <h4 className="text-xs font-medium text-primary mb-2 uppercase tracking-wide">Physics</h4>
                          <div className="max-h-32 overflow-y-auto space-y-1 pr-2 custom-scrollbar">
                            {[
                              "Quantum Mechanics",
            ...
            <div className="flex items-center gap-4">
              ...
              <div className="text-sm text-muted-foreground flex items-center gap-4">
                ...
                <button
                  ...
                  title="Reset ELO, Stats, and Filters"
