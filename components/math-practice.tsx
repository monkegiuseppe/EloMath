...
const mathProblems = allProblems.filter((problem) => mathCategories.includes(problem.category))
...
const AnswerFormatGuide = () => (
  <>
    ...
    <div className="space-y-2">
      <h3 className="text-md font-semibold text-primary">2. Mathematical Constants</h3>
      <ul className="list-disc list-inside space-y-1 pl-2 text-muted-foreground">
        <li>
          Pi: <code className="bg-muted px-1 rounded">π</code> or <code className="bg-muted px-1 rounded">pi</code>
        </li>
        <li>
          Euler's number: <code className="bg-muted px-1 rounded">e</code>
        </li>
      </ul>
    </div>
  </>
...
export default function MathPractice({ onBack }) {
  ...
  const [selectedCategories, setSelectedCategories] = useState(mathCategories)
  ...
  const getNewProblem = (currentElo) => {
    ...
    if (availableProblems.length === 0) {
      setCurrentProblem({
        ...
        topic: "System",
        difficulty: 0,
        problem: "No math problems available for the selected categories. Please select a category to begin.",
        answer: "",
      })
      return
    }
    ...
    if (eligibleProblems.length === 0) {
      setCurrentProblem({
        ...
        topic: "System",
        difficulty: 0,
        problem: "You have seen all available math problems for this filter. Please select more categories or reset.",
        answer: "",
      })
      return
    }
  ...
  useEffect(() => {
    const initialElo = getPlayerElo("math")
    setUserElo(initialElo)

    const savedCategories = localStorage.getItem("selectedMathCategories")
    if (savedCategories) {
      try {
        const parsed = JSON.parse(savedCategories)
        const validSaved = parsed.filter((c) => mathCategories.includes(c))
        if (validSaved.length > 0) {
          setSelectedCategories(validSaved)
        }
      } catch (e) {
        console.error("Failed to parse saved math categories:", e)
        localStorage.removeItem("selectedMathCategories")
      }
    }
  ...
  const handleSubmit = (e) => {
    ...
    const newElo = updatePlayerElo(currentProblem.difficulty, correct, "math")
    ...
    if (correct) {
      ...
      setFeedback({
        type: "correct",
        message: `Correct! Math ELO ${eloChange >= 0 ? "+" : ""}${eloChange}`,
      ...
      setFeedback({
        type: "incorrect",
        message: `Incorrect. Math ELO ${eloChange}`,
        correctAnswer: currentProblem.answer,
      })
    }
  }
  ...
  const handleResetElo = () => {
    if (window.confirm("Are you sure you want to reset your Math ELO, session stats, and category filters?")) {
      setUserElo(STARTING_ELO)
      localStorage.removeItem("playerElo_math")
      setSessionStats({ correct: 0, incorrect: 0, skipped: 0 })
      setSelectedCategories(mathCategories)
      localStorage.removeItem("selectedMathCategories")
  ...
  const handleCategoryToggle = (category) => {
    ...
    localStorage.setItem("selectedMathCategories", JSON.stringify(newSelectedCategories))
            ...
            <div className="flex items-center gap-4">
              ...
              <div className="relative">
                <button
                  ...
                  title="Filter math categories"
                >
                  <SlidersHorizontal size={16} className="inline mr-2" />
                  Math Categories ({selectedCategories.length}/{mathCategories.length})
                </button>
                ...
                <AnimatePresence>
                  {isCategorySelectorOpen && (
                    ...
                    >
                      <h3 className="text-sm font-semibold text-foreground mb-3">Mathematics Categories</h3>
            ...
            <div className="flex items-center gap-4">
              ...
              <div className="text-sm text-muted-foreground flex items-center gap-4">
                ...
                <button
                  ...
                  title="Reset Math ELO, Stats, and Filters"
